...
- 출력 형태:
{
  ...
  "recapture":false,"recapture_reasons":[]
}
...
- 명칭 매핑/예외:
  · 급여총액 ← (급여, 임원급여, 직원급여, 상여금, 임금, 잡급) 중 문서에 있는 것을 전부 합산
    → status="exception", note="합산: (실제 합산된 항목명 나열)". 잡급도 반드시 포함.
    (퇴직급여는 급여총액에 합치지 말 것 — 별도 항목)
  · 용수비(수도비) ← '수도광열비'(수도광역비 표기 포함) 또는 '가스수도료'
    → status="exception", note="원문 '○○○', 가스/광열 포함"
  · 임차료 ← '임차료' 또는 '지급임차료' → 순수 동의어, status="ok"(특이 note 없음)
  · 원재료비 ← '원재료비' (+'포장비'가 합산돼 있으면) → 포장비 포함 시 status="exception",
    note="포장비 포함". 원재료비 단독이면 status="ok".
  · 운반·하역·보관비 ← '운반비'만   → status="exception", note="운반비만 반영, 하역·보관 제외"
  · 세금과공과 ← 세금과공과금/제세공과금 → status="exception", note="원문 '세금과공과금'"
  · 경상연구개발비 ← 경상개발비/연구개발비
...
[검산 / 재촬영 판정] 추출 후, 인쇄된 소계와 네가 추출한 구성요소의 합이 맞는지 검산한다.
행 밀림(라벨↔값 한 칸 어긋남)은 잘리지 않은 멀쩡한 표에서도 발생하므로 반드시 확인한다.
- 제조원가명세서:
  · 제조경비 소계 == Σ(제조경비 항목들)
  · 노무비 소계 == 급여 + 임금 + 퇴직급여
  · 원재료비 == 기초원재료 + 당기매입 − 기말원재료
- 손익계산서:
  · 판관비 합계 == Σ(판관비 소항목)
  · 영업이익 == 매출총이익 − 판관비 합계
- 재무상태표:
  · Σ(유형자산 순액) == 인쇄된 유형자산 합계
검산이 어긋나면(원 단위로 의미 있게 차이나면) recapture=true 로 한다.
recapture_reasons 문구 규칙(중요):
- 회계를 모르는 50대 일반인(어머님)도 한 번에 이해할 쉬운 말로 쓴다.
- 숫자·계정과목명·"소계/검산/드리프트" 같은 전문용어를 쓰지 말 것.
- 원인은 대개 "종이가 구겨져서 숫자 줄이 한 칸씩 밀린 것"이다. 이 점을 알려주고
  "평평하게 펴서 다시 찍어 올려주세요"라고 안내한다.
- 사진이 잘려 일부가 안 보이면 "사진 아래쪽(또는 옆)이 잘려서 일부 숫자가 안 보여요.
  전체가 다 나오게 다시 찍어주세요" 식으로.
- 각 사유는 1문장, 부드러운 존댓말.
예시:
  "숫자 줄이 한 칸씩 밀려서 합계가 맞지 않아요. 종이가 구겨져 있으면 그럴 수 있어요. 평평하게 펴서 다시 찍어 올려주세요."
  "사진 아래쪽이 잘려서 일부 항목이 안 보여요. 표 전체가 다 나오게 다시 찍어주세요."
모두 맞으면 recapture=false, recapture_reasons=[]. 모든 doc_type 출력에 이 두 필드를 포함한다.
(balance_sheet 출력에도 "recapture","recapture_reasons" 를 포함)
